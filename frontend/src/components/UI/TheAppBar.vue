<template>
  <div>
    ...
    <v-app-bar clipped-left dense app color="primary" dark class="d-print-none" :bottom="isMobile">
      ...
      <router-link to="/">
        <!-- <v-btn icon class="titleLogo">
          <v-icon size="100" class="titleLogo-icon"> {{ $globals.icons.primary }} </v-icon>
          
        </v-btn> -->
        <img src="./CivitatesCenaSimplifedTransparent.png" style="height:50px; margin-top:5px;" />
      </router-link>

      <!-- <div v-if="!isMobile" btn class="pl-2">
        <v-toolbar-title style="cursor: pointer" @click="$router.push('/')"> Civitates CENA </v-toolbar-title>
      </div> -->
...
// const TitleLogo = Document.querySelector
// TitleLogo.options.props.ripple.default = false
