...
To use the bundled configuration file, simply rename <code>cena.subdomain.conf.sample</code> in the proxy-confs folder to <code>cena.subdomain.conf</code>. Currently, you will have to change the $upstream_app and $upstream_port to cena-frontend and port 3000. This will be added to the bundled config files once the beta is released.
Alternatively, you can create a new file <code>cena.subdomain.conf</code> in proxy-confs with the following configuration:
...
```yaml
    ...
    listen [::]:443 ssl http2;
    	...
    	location / {
        	...
        	set $upstream_app cena-frontend;
