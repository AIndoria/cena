...
!!! warning
    Be sure to use the [Nightly version](https://nightly.cena.io/) of the docs to ensure you're up to date with
    the latest changes.
...
=== "Windows"
    ...
    ``` powershell
    # Terminal # 1
	poetry run python cena/db/init_db.py # Initialize the database
	poetry run python cena/app.py # start application
