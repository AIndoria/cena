...
from cena.services.backups_v2.alchemy_exporter import AlchemyExporter
from cena.services.backups_v2.backup_file import BackupFile
...
class BackupV2(BaseService):
    ...
    def _sqlite(self) -> None:
        ...
        shutil.copy(db_file, f"cena_{timestamp}.bak.db")
    ...
    def backup(self) -> Path:
        ...
        exclude = {"cena.db", "cena.log", ".secret"}
        exclude_ext = {".zip"}
        exclude_dirs = {"backups", ".temp"}

        timestamp = datetime.datetime.now().strftime("%Y.%m.%d.%H.%M.%S")

        backup_name = f"cena_{timestamp}.zip"
