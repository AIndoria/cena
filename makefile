...
setup-model: ## 🤖 Get the latest NLP CRF++ Model
	...
	curl -L0 https://github.com/cena-recipes/nlp-model/releases/download/v1.0.0/model.crfmodel --output ./cena/services/parser_services/crfpp/model.crfmodel


clean-data: ## ⚠️  Removes All Developer Data for a fresh server start
	rm -r ./dev/data/recipes/
	rm -r ./dev/data/users/
	rm -f ./dev/data/cena*.db
	rm -f ./dev/data/cena.log
	rm -f ./dev/data/.secret

clean-pyc: ## 🧹 Remove Python file artifacts
	find ./cena -name '*.pyc' -exec rm -f {} +
	find ./cena  -name '*.pyo' -exec rm -f {} +
	find ./cena  -name '*~' -exec rm -f {} +
	find ./cena  -name '__pycache__' -exec rm -fr {} +
