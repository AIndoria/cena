...
from cena.core.config import CWD, DATA_DIR, AppDirectories, AppSettings, determine_data_dir, determine_secrets
...
def test_default_settings(monkeypatch):
    ...
    assert app_settings.POSTGRES_USER == "cena"
    assert app_settings.POSTGRES_PASSWORD == "cena"
...
def test_non_default_settings(monkeypatch):
    ...
    monkeypatch.setenv("POSTGRES_USER", "cena-test")
    monkeypatch.setenv("POSTGRES_PASSWORD", "cena-test")
    ...
    assert app_settings.POSTGRES_USER == "cena-test"
    assert app_settings.POSTGRES_PASSWORD == "cena-test"
...
def test_default_connection_args(monkeypatch):
    ...
    app_settings = AppSettings()
    assert re.match(r"sqlite:////.*cena/dev/data/cena_v0.5.0.db", app_settings.DB_URL)
